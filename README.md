...
**Окно настроек (Windows / Linux):** секция **«Статус»** — цветной индикатор, текст состояния и uptime (обновляется автоматически). Полная проверка TCP и детали — в меню трея **«Статус и проверка TCP…»**. В блоке **«Обновления»** — время последней проверки GitHub.
