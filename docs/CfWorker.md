...
Дополнительно тот же Worker можно использовать для [Telegram Bot API](./BotApi.md): туннель идёт через уже существующий путь `/apiws?dst=…` — менять код Worker не нужно.
