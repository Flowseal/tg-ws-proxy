...
> [!NOTE]
>
> В Linux-настройках доступен tray-автозапуск: он создаёт `~/.config/autostart/tg-ws-proxy.desktop` и подходит для GUI-режима.
> XDG-автозапуск поддерживается только для упакованной Linux-сборки (`.deb` / `TgWsProxy_linux_amd64`).
