...
В Linux-настройках доступен обычный tray-автозапуск:

- **Автозапуск tray при входе в сессию** - создаёт `~/.config/autostart/tg-ws-proxy.desktop` и подходит для GUI-режима

> [!NOTE]
>
> XDG-автозапуск tray поддерживается только для упакованной Linux-сборки (`.deb` / `TgWsProxy_linux_amd64`).
...
```json
{
  ...
  "cfproxy_user_domain": "",
  "appearance": "auto",
  "linux_gui_autostart": false
}
```

Ключ **`check_updates`** - при `true` при запросе к GitHub сравнивается версия с последним релизом (только уведомление и ссылка на страницу загрузки). На Windows в конфиге может быть **`autostart`** (автозапуск при входе в систему). На Linux используется **`linux_gui_autostart`**, а фактическое состояние дополнительно синхронизируется с XDG autostart.
